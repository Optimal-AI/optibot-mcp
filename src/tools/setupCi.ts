...
export async function handleSetupCi(input: { name?: string }): Promise<ToolResult> {
    // CI guard FIRST — a user in CI without auth would otherwise see "Not
    // authenticated" and waste a login round-trip before hitting the CI
    // refusal. Mirror the CLI's `setup ci` ordering.
    if (isCiEnvironment()) {
        return {
            content: [{
                type: 'text',
